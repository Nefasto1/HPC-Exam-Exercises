...
int main(int argc, char** argv) {
    ...
    if (rank == 0){
        ...
        if (size > 2) 
            MPI_Send(buffer, n, MPI_INT, 2, 0, MPI_COMM_WORLD);
        // printf("Root has sent the message to all the processes\n");
    }
    // If non-root process
    else {
        // Obtain the data
        int parent = (rank - (1 + (rank%2 == 0))) / 2;
        MPI_Recv(buffer, n, MPI_INT, parent, 0, MPI_COMM_WORLD, &status);
        ...
        if (rank*2+1 <= size-1)
            MPI_Send(buffer, n, MPI_INT, 2*rank+1, 0, MPI_COMM_WORLD);
        if (rank*2+2 <= size-1)
            MPI_Send(buffer, n, MPI_INT, 2*rank+2, 0, MPI_COMM_WORLD);
    }
