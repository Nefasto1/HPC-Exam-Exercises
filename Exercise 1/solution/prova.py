...
df = pd.read_csv(f"results_thin_merged.txt", sep="\t")
df_cor = pd.read_csv(f"latency_different_core_preprocessed.txt", sep=",")
df_soc = pd.read_csv(f"latency_different_socket_preprocessed.txt", sep=",")
df_nod = pd.read_csv(f"latency_different_node_preprocessed.txt", sep=",")
...
for type in ["broadcast", "scatter"]:
    for n in df["Size"].unique():
        ...
        plt.savefig(f"prova {type} Size={n}.png")
        plt.close()
